...
(function($, undefined) {
    ...
    */
    ...
    $.fn.terminal = function(init_eval, options) {
        ...
        function draw_line_with_staggered_key_strokes_loop(string, container_div, output_length)
        {
          container_div.html(string.substr(0, output_length++));
          if(output_length < string.length+1)
          {
            setTimeout(function(){draw_line_with_staggered_key_strokes_loop(string, container_div, output_length)}, Math.random() * settings.keystroke_max_delay_milliseconds);
          } else {
            command_line.enable();
          }
        }

        function draw_line_with_staggered_key_strokes(string) {
            command_line.disable();
            div = $('<div> </div>');
            output.append(div);
            div.width('100%');
            scroll_to_bottom();
            draw_line_with_staggered_key_strokes_loop(string, div, 0);
            return div;
        }
        ...
        $.extend(self, {
            ...
            resize: function(width, height) {
                if (settings.keystroke_max_delay_milliseconds) {
                  // If delaying keystrokes, don't rebuild here.
                  return;
                }
            ...
            echo_with_staggered_keystrokes: function(line) {
                if(!settings.keystroke_max_delay_milliseconds) {
                  return(echo(line));
                }
                lines.push(line);
                return draw_line_with_staggered_key_strokes(line);
            },
